...
@interface mememeViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
- (void)viewDidLoad {
    ...
    NSLog(@"\n\n\n\n%@",((JMSGUser*)_conversation.target));
    NSLog(@"\n\n\n\n%@",((JMSGUser*)_conversation.target).nickname);
//    [_conversation allMessages:^(id resultObject, NSError *error) {
//        NSLog(@"\n\n%@",resultObject);
//    } ];
//    
    //  NSString*str=[[NSString alloc] initWithData:_conversation encoding:NSUTF8StringEncoding];
    
   
    // NSDictionary *content = [NSJSONSerialization JSONObjectWithData:_conversation options:NSJSONReadingMutableContainers error:nil];
    self.navigationItem.title = ((JMSGUser*)_conversation.target).nickname;
    //设置self.view背景颜色
...
- (UIView *)yuyinView:(NSInteger)logntime from:(BOOL)fromSelf withIndexRow:(NSInteger)indexRow  withPosition:(int)position{
    ...
    button.tag = indexRow;
    if(fromSelf)
        button.frame =CGRectMake(width-position-yuyinwidth, 10, yuyinwidth, 54);
    else
